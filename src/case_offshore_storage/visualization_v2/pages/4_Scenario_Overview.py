...
# Read data
all_cases = pd.read_excel('src/case_offshore_storage/visualization_v2/data/Summary_Plotting6_processed.xlsx')
...
max_abatement = all_cases[all_cases['Emission Reduction'] != 'Min emissions']['abatemente_cost'].max()*1.1
min_abatement = all_cases[all_cases['Emission Reduction'] != 'Min emissions']['abatemente_cost'].min()
# max_abatement = all_cases['abatemente_cost'].max()
# min_abatement = all_cases['abatemente_cost'].min()

# Filter data
all_cases = all_cases[(all_cases['Case'] != 'Baseline') & (all_cases['Emission Reduction2'] != 'Min emissions')]
...
#
#     plot_data = all_cases[all_cases['Emission Reduction2'] == emission_red_case]
...
for case in sort_cases:
    plot_case = all_cases[(all_cases['Emission Reduction2'] == case)]
