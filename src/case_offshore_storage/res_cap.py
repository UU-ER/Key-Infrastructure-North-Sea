...
class Res_Cap(Res):
    ...
    def fit_technology_performance(self, node_data):
        node = node_data.name
        max_caps = pd.read_csv('./case_study_data/clean_data/max_re_cap/max_re_nodes.csv')

        # read in data
        if self.name == 'Offshore_Wind':
            self.size_max = min(max_caps[max_caps['Node'] == node][
                'RemainingPotential_Wind_off'].values[0], 250000/2)
            capacity_factor = pd.read_csv('./case_study_data/clean_data/capacity_factors/wind_offshore.csv')
        elif self.name == 'Onshore_Wind':
            self.size_max = min(max_caps[max_caps['Node'] == node][
                'RemainingPotential_Wind_on'].values[0], 100000)
            capacity_factor = pd.read_csv('./case_study_data/clean_data/capacity_factors/wind_onshore.csv')
        elif self.name == 'PV':
            self.size_max = min(max_caps[max_caps['Node'] == node][
                'RemainingPotential_PV'].values[0], 400000)
            capacity_factor = pd.read_csv('./case_study_data/clean_data/capacity_factors/pv.csv')
